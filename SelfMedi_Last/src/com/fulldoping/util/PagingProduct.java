...
public class PagingProduct {
	...
	//총 게시글 수를 입력하는 생성자
	public PagingProduct(int totalCount) {
	...
	public PagingProduct(int totalCount, int curPage) {
		setTotalCount(totalCount);
		setCurPage(curPage);
	...
	public PagingProduct(int totalCount, int curPage, int listCount) {
		setTotalCount(totalCount);
		setCurPage(curPage);
	...
	public PagingProduct(int totalCount, int curPage, int listCount, int pageCount) {
		setTotalCount(totalCount);
		setCurPage(curPage);
		setListCount(listCount);
	...
	private void makePaging() {
		...
		if(listCount == 0)	setListCount(6);	//화면에 보여질 게시글 수를 기본 10개로 설정한다
		if(pageCount == 0)	setPageCount(5);	//화면에 보여질 페이지 수를 기본 10페이지로 설정한다
