package com.fulldoping.admin.product.paging;

public class AdProductPaging {
	...
	public AdProductPaging() { }
	
	//총 게시글 수를 입력하는 생성자
	public AdProductPaging(int totalCount) {
	...
	public AdProductPaging(int totalCount, int curPage) {
		setTotalCount(totalCount);
		setCurPage(curPage);
	...
	public AdProductPaging(int totalCount, int curPage, int listCount) {
		setTotalCount(totalCount);
		setCurPage(curPage);
	...
	public AdProductPaging(int totalCount, int curPage, int listCount, int pageCount) {
		setTotalCount(totalCount);
		setCurPage(curPage);
		setListCount(listCount);
	...
	private void makePaging() {
		...
		if(listCount == 0)	setListCount(4);	//화면에 보여질 게시글 수를 기본 10개로 설정한다
