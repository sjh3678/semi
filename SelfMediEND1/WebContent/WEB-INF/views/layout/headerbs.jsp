...
$(document).ready(function() {
	...
	$(document).on("click", "#basket", function() {
		if(session.getAttribute("login")) {
			$(location).attr("href", "/basket/view");
		} else {
			if( confirm("로그인 필요. 메인화면으로 이동합니다") ) {
				$(location).attr("href", "/");
			}
		}
	});
	
	$(document).on("click", "#selftestList", function() {
		if(session.getAttribute("login")) {
...
.quickmenu > ul > li {list-style-type: none;font-size: 15px;border-radius:5px;background-color: #b2bec3;}
