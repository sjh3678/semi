			...
			<c:forEach items="${myList }" var="testInfo" begin="0" end="4">
				<tr class="testList">
					...
					<td><c:if test="${testInfo.userGender eq 'Male'}">남성</c:if> <c:if
						...
						<td><button onclick='location.href="<%=request.getContextPath() %>/selftest/delete?selftestNo=${testInfo.selftestNo }";'>삭제</button></td>
<!-- 					<td><button type="button" id="btnDelete">삭제</button></td> -->
	...
	<button type="button" id="btnAdd">자가진단 추가</button>
<br><br>
