...
.nuthead{width:50%;margin:0;float:left; padding-top:30px;}
td{border: 1px solid #ccc;}
.producttd{height:50px;vertical-align: middle;}
.title{text-align: center; background-color: #345EE6; color: #fff;}
meter {
...
<div class="container">
	<h3 class="title">제품 비교</h3>
	<c:forEach items="${productInfo }" var="productInfo">
		<div class="producttb">
			<table class="product" style="cellspacing: 0; border-collapse: collapse;">
				<thead>
					<tr>
					...
					<tr>
						<td class="producttd" colspan="5">${productInfo.productName }</td>
					</tr>
					<tr>
						<td style="width:146.25px;background-color: #EEE;">제조사 </td>
						<td class="producttd" colspan="4">${productInfo.manuCom }</td>
					</tr>
					<tr>
						<td class="producttd" colspan="1" style="width: 146.25px;background-color: #EEE;">타입</td>
						<td class="typroducttdpe" colspan="1" style="width: 146.25px;">${productInfo.type }</td>
						<td style="width:146.25px;background-color: #EEE;">별점</td>
	...
	<div class="nuthead">
		<table>
			<tr class="nut" style="height:50px;">
				<td style="width:80px;background-color: #EEE;">함유 영양소</td>
				<td  colspan="2" style="width:285px;background-color: #EEE;">권장량 비교 그래프</td>
				<td style="width:110px;background-color: #EEE;">함량</td>
				<td style="width:110px;background-color: #EEE;">권장량</td>	
			</tr>
		</table>
	</div>
			...
			<div class="nuttb" >
				<table style="cellspacing: 0; border-collapse: collapse;">
					<tr style="height:50px;">
						<td class="nuttd" style="width:80px;">${nik.nk.nutKind }</td>
						<td class="nuttd" colspan="2" style="width:285px;"><meter
							low='${nik.nk.rcmAdult.replaceAll("[^0-9]", "")*0.6 }'
							high='${nik.nk.rcmAdult.replaceAll("[^0-9]", "")*1.5 }' 
							min='0'
							max='${nik.nk.rcmAdult.replaceAll("[^0-9]", "")*2 }'
							optimum='${nik.nk.rcmAdult.replaceAll("[^0-9]", "") }'
							value='${nik.ni.nutContent.replaceAll("[^0-9]", "") }'> </meter></td>
						<td class="nuttd" style="width:110px;">${nik.ni.nutContent }</td>
						<td class="nuttd" style="width:110px;">${nik.nk.rcmAdult }</td>
					</tr>
				</table>
			</div>
		</c:forEach>
			...
			<div class="nuttb">
				<table style="cellspacing: 0; border-collapse: collapse;">
					<tr style="height:50px;">
						<td class="nuttd" style="width:80px;">${nik.nk.nutKind }</td>
						<td class="nuttd" colspan="2" style="width:285px;"><meter
							low='${nik.nk.rcmAdult.replaceAll("[^0-9]", "")*0.5 }'
							high='${nik.nk.rcmAdult.replaceAll("[^0-9]", "")*1.5 }' 
							min='0'
							max='${nik.nk.rcmAdult.replaceAll("[^0-9]", "")*2.1 }'
							optimum='${nik.nk.rcmAdult.replaceAll("[^0-9]", "")*1 }'
							value='${nik.ni.nutContent.replaceAll("[^0-9]", "") }'> </meter></td>
						<td class="nuttd" style="width:110px;">${nik.ni.nutContent }</td>
						<td class="nuttd" style="width:110px;">${nik.nk.rcmAdult }</td>
					</tr>
